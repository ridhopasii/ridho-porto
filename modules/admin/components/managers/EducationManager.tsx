...
export default function EducationManager() {
  ...
  const list = useAdminList({
    items: educations,
    searchFields: ["institution", "degree", "major", "location"],
    pageSize: PAGE_SIZE,
  });
  ...
  const handleMove = async (idx: number, dir: "up" | "down") => {
    ...
    // Try to persist — silently OK if column doesn't exist yet
    const results = await Promise.allSettled([
      fetch("/api/admin/education", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: withOrder[idx].id, sort_order: idx }) }),
      fetch("/api/admin/education", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: withOrder[targetIdx].id, sort_order: targetIdx }) }),
    ]);
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Yakin ingin menghapus ${list.selected.size} pendidikan?`)) return;
    setBulkDeleting(true);
    await Promise.all(Array.from(list.selected).map((id) => fetch(`/api/admin/education?id=${id}`, { method: "DELETE" })));
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <AdminSearchBar value={list.search} onChange={list.setSearch} placeholder="Cari institusi, jurusan, gelar..." total={educations.length} filtered={list.filtered.length} />
      ...
      {loading ? (
        <div className="animate-pulse grid grid-cols-1 gap-3 lg:grid-cols-2">{[1, 2, 3, 4].map((i) => <div key={i} className="h-24 rounded-xl bg-neutral-100 dark:bg-neutral-800" />)}</div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            {list.paginated.map((item) => {
              const actualIdx = educations.findIndex((e) => e.id === item.id);
              return (
                <div key={`edu-${item.id}`} className="group flex items-start gap-3 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                  ...
                  {!isSearching && (
                    <div className="flex flex-shrink-0 flex-col gap-0.5 pt-1">
                      ...
                      <button onClick={() => handleMove(actualIdx, "down")} disabled={actualIdx === educations.length - 1} className="rounded p-0.5 text-neutral-400 hover:bg-neutral-100 disabled:opacity-20 dark:hover:bg-neutral-800 transition-colors">
                        <TbArrowDown size={13} />
                      </button>
                    </div>
                  )}
            ...
            {list.paginated.length === 0 && (
              <div className="col-span-2 py-12 text-center text-neutral-500">
                {list.search ? `Tidak ada hasil untuk "${list.search}"` : "Belum ada data pendidikan."}
              </div>
            )}
          </div>
