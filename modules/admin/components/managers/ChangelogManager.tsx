...
export default function ChangelogManager() {
  ...
  const list = useAdminList({ items, searchFields: ["version", "description"], pageSize: PAGE_SIZE });
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Yakin ingin menghapus ${list.selected.size} entri changelog?`)) return;
    setBulkDeleting(true);
    await Promise.all(Array.from(list.selected).map((id) => fetch(`/api/admin/changelogs?id=${id}`, { method: "DELETE" })));
    toast.success(`${list.selected.size} entri berhasil dihapus`);
    list.clearSelected();
    fetchData();
  ...
  return (
    <div className="space-y-4 p-6">
      {isModalOpen && <ChangelogFormModal item={editingItem} onClose={() => setIsModalOpen(false)} onSuccess={() => { setIsModalOpen(false); fetchData(); }} />}

      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <AdminSearchBar value={list.search} onChange={list.setSearch} placeholder="Cari versi, deskripsi..." total={items.length} filtered={list.filtered.length} />
        ...
        <>
          ...
          <div className="space-y-3">
            {list.paginated.map((item) => (
              <div key={item.id} className="group flex items-start gap-3 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div className="flex-1">
                  <div className="mb-2 flex items-center gap-3">
                    <span className="rounded bg-blue-100 px-2 py-0.5 text-xs font-bold text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{item.version}</span>
                    <span className="text-sm text-neutral-500">{item.date}</span>
                  </div>
                  <p className="whitespace-pre-wrap text-sm text-neutral-600 dark:text-neutral-400">{item.description}</p>
                </div>
            ...
            {list.paginated.length === 0 && <div className="py-12 text-center text-neutral-500">{list.search ? `Tidak ada hasil untuk "${list.search}"` : "Belum ada changelog."}</div>}
          </div>
