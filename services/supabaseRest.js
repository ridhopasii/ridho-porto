...
async function request(
  table,
  { method = 'GET', query = {}, body, headers = {}, accessToken, serviceRole } = {}
) {
  if (!isConfigured()) {
  ...
  const resolvedServiceRole =
    serviceRole !== undefined
      ? serviceRole
      : !isRead && Boolean(SUPABASE_SERVICE_ROLE_KEY) && !accessToken;
...
async function selectOne(
  table,
  { select = '*', filters = {}, orderBy, accessToken, serviceRole } = {}
) {
