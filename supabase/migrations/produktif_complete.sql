-- ============================================================
-- PRODUKTIF COMPLETE DATABASE SCHEMA
-- Imported from produktifkuu (Tabungan Umroh, One Year Plan, Monthly Tracker, Daily Habits)
-- ============================================================

-- 1. TABEL UTAMA: DAILY PRODUCTIVITY LOG (sudah ada, pastikan lengkap)
...
-- 2. TABEL SITE SETTINGS (untuk semua konfigurasi yang bisa diedit admin)
...
-- 3. TABEL TABUNGAN UMROH (dengan kolom per orang seperti di HTML asli)
...
-- 4. TABEL ONE YEAR PLAN (4 kategori dari HTML asli)
...
-- 5. TABEL MONTHLY TRACKER (ibadah sunnah per hari)
CREATE TABLE IF NOT EXISTS "MonthlyTracker" (
    "id" SERIAL PRIMARY KEY,
    "date" DATE NOT NULL,
    "checklist" JSONB NOT NULL DEFAULT '{}',
    -- checklist berisi: sholat5waktu, subuhBerjamaah, dhuha, tahajud, 
    -- bacaQuran, hafalanQuran, puasaSunnah, sedekah, dzikirPagi, dzikirSore,
    -- workout, minum2L, tidurCukup, bacaBuku, belajarSkill, dll
    "notes" TEXT,
    "createdAt" TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE("date")
);

-- 6. TABEL HABITS (konfigurasi kebiasaan yang bisa diedit admin)
...
-- ============================================================
-- RLS POLICIES (Allow all untuk private personal dashboard)
-- ============================================================
...
-- ============================================================
-- DATA SEED: DAILY PRODUCTIVITY TEMPLATE (Block 1, 2A, 2B, 2Sunday, 3)
-- ============================================================
...
-- ============================================================
-- DATA SEED: LIFE HUB CONFIG (dari One Year Plan & produktifkuu)
-- ============================================================
INSERT INTO "SiteSettings" ("key", "value") VALUES (
    'life_hub_config',
    '{
        "yearly": [
            "Menyelesaikan 12 buku dalam setahun",
            "Menambah pemahaman tentang Kepribadian dalam Islam",
            "Belajar hal baru setiap bulan",
            "Belajar management waktu yang lebih baik",
            "Belajar menabung & capai target tabungan Umroh",
            "Menjadi pendengar yang lebih baik",
            "Semangat menyebarkan kebaikan",
            "Menulis 50 affirmasi positif",
            "Menghasilkan 360 konten dalam 1 Tahun",
            "Menambah hafalan Al-Quran",
            "Menambah pengetahuan bahasa Inggris dan Arab",
            "Rajin melaksanakan puasa sunnah",
            "Lebih banyak konsumsi buah dan sayuran",
            "Menghindari minuman bergula tinggi"
        ],
        "savings": [
            {"name": "Tabungan Umroh Diri Sendiri", "target": "30000000", "current": "0", "icon": "🕌", "targetDate": "Desember 2025"},
            {"name": "Tabungan untuk Mahram", "target": "30000000", "current": "10000000", "icon": "👤", "targetDate": "2026"},
            {"name": "Tabungan untuk Keluarga", "target": "30000000", "current": "15000000", "icon": "👨‍👩‍👧‍👦", "targetDate": "2026"},
            {"name": "Tabungan untuk Orang Lain", "target": "30000000", "current": "25000000", "icon": "🤲", "targetDate": "2026"}
        ],
        "habits": [
            ...
            "Workout 30 Menit",
            "Baca Buku 35 Halaman/Pekan",
            "Buat 1 Konten Per Hari",
            "Minum Air 2 Liter/Hari"
        ],
        "quote": "Bismillah... Bukan ingin menjadi lebih baik dari orang lain, tapi aku hanya ingin lebih baik dari diriku yang dulu.",
        "motivation": "Siapa pun kita di masa lalu bukan berarti kita tidak berhak menjadi muslim yang lebih baik, berhijrahlah, berubahlah, walau sejengkal demi sejengkal."
    }'
) ON CONFLICT ("key") DO NOTHING;
...
-- ============================================================
-- DATA SEED: ONE YEAR PLAN 2026 (dari HTML asli)
-- ============================================================
...
(2026, 'mindset', 'Membaca Kitab (isi sendiri)', NULL, NULL, NULL, FALSE, 9),
-- SKILL
...
-- ============================================================
-- DATA SEED: TABUNGAN UMROH (data awal dari HTML - Januari 2026)
-- ============================================================
INSERT INTO "TabunganUmroh" ("month", "year", "category", "amount", "target", "notes") VALUES
-- Data yang sudah ada di HTML
('January', 2026, 'mahram', 10000000, 30000000, 'Setoran awal dari HTML asli'),
('January', 2026, 'keluarga', 15000000, 30000000, 'Setoran awal dari HTML asli'),
('January', 2026, 'orang_lain', 25000000, 30000000, 'Setoran awal dari HTML asli'),
('January', 2026, 'diri_sendiri', 0, 30000000, 'Belum ada setoran')
ON CONFLICT ("month", "year", "category") DO NOTHING;

SELECT 'Database migration complete! All tables created and data seeded from produktifkuu.' AS status;
