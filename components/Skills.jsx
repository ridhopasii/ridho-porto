          ...
          {Array.isArray(skills) &&
            skills.map((skill, idx) => (
              <Link
                ...
                className="group p-6 bg-white/5 border border-[var(--border-subtle)] rounded-3xl hover:border-blue-500/50 hover:bg-blue-500/5 transition-all text-center flex-shrink-0 w-[40vw] sm:w-[25vw] md:w-[18vw] lg:w-[14vw] snap-center"
              >
                <div className="w-12 h-12 mx-auto mb-4 text-muted-foreground group-hover:text-blue-500 group-hover:scale-110 transition-all">
                  <Cpu size={48} strokeWidth={1} />
                </div>
                <h3 className="font-bold text-sm tracking-widest uppercase text-foreground group-hover:text-blue-500 transition-colors">
                  {skill.name}
                </h3>
                <p className="text-[10px] text-muted-foreground mt-1 uppercase group-hover:text-blue-500/50 transition-colors font-black tracking-widest">
                  {skill.level || 'Expert'}
                </p>
              </Link>
            ))}
