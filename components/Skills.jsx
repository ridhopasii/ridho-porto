...
export default function Skills({ skills }) {
  return (
    <section id="keterampilan" className="py-24 px-6 relative overflow-hidden bg-background">
          ...
          {Array.isArray(skills) &&
            skills.map((skill, idx) => (
              ...
              >
                ...
                <h3 className="font-bold text-sm tracking-widest uppercase text-foreground group-hover:text-accent transition-colors">
                  {skill.name}
                </h3>
                <p className="text-[10px] text-muted-foreground mt-1 uppercase group-hover:text-accent/50 transition-colors font-black tracking-widest">
                  {skill.level || 'Expert'}
                </p>
              </Link>
            ))}
      ...
      {/* Background Decor */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-accent/5 rounded-full blur-[120px] z-0"></div>
    </section>
  );
}
