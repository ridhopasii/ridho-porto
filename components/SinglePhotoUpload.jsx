...
export default function SinglePhotoUpload({
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {/* Status */}
      {value ? (
        <span className="text-[10px] text-accent font-black uppercase tracking-widest">✓ Foto Tersimpan</span>
      ) : (
        <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Belum ada foto</span>
      )}

      {/* Upload Button */}
      <label className="cursor-pointer px-4 py-2 bg-white/5 border border-[var(--border-subtle)] rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-400 hover:border-accent/50 hover:text-accent transition-all flex items-center gap-2">
        {uploading ? (
      ...
      <div className="w-full">
        <input
          ...
          className="w-full bg-white/5 border border-[var(--border-subtle)] rounded-xl px-3 py-2 text-[10px] text-gray-400 focus:outline-none focus:border-accent transition-all"
        />
      </div>
    </div>
  );
