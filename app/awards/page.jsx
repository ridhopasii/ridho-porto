    ...
    <div className="min-h-screen bg-background text-foreground font-jakarta selection:bg-yellow-500/30">
      ...
      <section className="pb-32 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {awards?.map((award) => (
            <div key={award.id} className="p-6 bg-white/5 border border-[var(--border-subtle)] rounded-[2.5rem] group hover:border-yellow-500/30 transition-all flex flex-col h-[26rem]">
              ...
              <Link href={`/awards/${award.slug}`}>
                <h3 className="text-lg font-bold text-foreground mb-1 group-hover:text-yellow-500 transition-colors line-clamp-2">
              ...
              <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-white/5">
                ...
                {award.proofUrl && (
                  <a href={award.proofUrl} target="_blank" className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-yellow-500 hover:text-foreground transition-colors">
