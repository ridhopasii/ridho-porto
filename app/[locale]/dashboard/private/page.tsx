...
export async function generateMetadata({
  ...
  return {
    title: `Private Hub - ${t("title")} ${METADATA.exTitle}`,
    description: t("description"),
    alternates: { canonical: `${(process.env.DOMAIN || "https://ridhorobbipasi.my.id")}/${locale}/dashboard/private` },
  };
}

const PrivateDashboardPage = async ({ params }: Props) => {
  const { locale } = await params;
