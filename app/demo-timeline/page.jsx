    ...
    <div className="min-h-screen bg-background text-foreground font-jakarta">
      ...
      <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto space-y-32">
        <div className="text-center">
          <h1 className="text-4xl font-black font-outfit uppercase mb-4 text-accent">Preview 3 Opsi Timeline</h1>
              ...
              <div key={item.id} className="flex flex-col md:flex-row gap-8 md:gap-20 mb-24 last:mb-0">
                ...
                <div className="md:w-1/3">
                  <div className="sticky top-32 flex items-center gap-4">
                    ...
                    <span className="text-xl md:text-3xl font-black tracking-tighter text-accent">{item.period}</span>
                  </div>
                </div>
                {/* Content Right Side */}
                <div className="md:w-2/3 pt-2">
                  <h3 className="text-2xl md:text-4xl font-bold mb-4">{item.title}</h3>
                  <p className="text-accent font-bold uppercase tracking-widest text-sm mb-6">{item.company}</p>
              ...
              <div key={item.id} className="border border-[var(--border-subtle)] rounded-3xl overflow-hidden bg-white/5 transition-all">
                ...
                >
                  <div className="flex items-center gap-6">
                    <span className="text-accent text-xs font-bold uppercase tracking-widest">{item.period}</span>
                  ...
                  {activeAcc === item.id && (
                    ...
                    >
                      <div className="pt-6 border-t border-[var(--border-subtle)]">
                        <p className="text-accent font-bold uppercase tracking-widest text-xs mb-4">{item.company}</p>
        ...
        <section className="border border-[var(--border-subtle)] p-8 md:p-16 rounded-[3rem] bg-white/[0.02]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 md:row-span-2 p-8 rounded-[2rem] bg-accent/10 border border-accent/20 flex flex-col justify-end min-h-[300px]">
              <div className="mt-auto">
                <span className="inline-block px-4 py-2 rounded-full bg-accent text-black font-bold text-xs uppercase tracking-widest mb-4">Latest: {dummyData[0].period}</span>
                <h3 className="text-3xl font-black mb-2">{dummyData[0].title}</h3>
                <p className="text-accent mb-4 font-bold uppercase text-xs tracking-widest">{dummyData[0].company}</p>
