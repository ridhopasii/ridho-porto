...
export async function GET(req: NextRequest) {
  const { data, error } = await supabaseServer.from("Gallery").select("*").order("date", { ascending: false });
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await supabaseServer.from("Gallery").insert(body);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PUT(req: NextRequest) {
  ...
  const { error } = await supabaseServer.from("Gallery").update(updates).eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true });
}
