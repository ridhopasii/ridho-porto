      ...
      <section className="pb-32 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {publications?.map((pub) => (
            <div key={pub.id} className="p-6 bg-white/5 border border-[var(--border-subtle)] rounded-[2.5rem] group hover:border-blue-500/30 transition-all flex flex-col h-[26rem]">
              ...
              <Link href={`/publications/${pub.slug}`}>
                <h3 className="text-lg font-bold text-foreground mb-1 group-hover:text-blue-500 transition-colors line-clamp-2">
              ...
              <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-white/5">
                <Link href={`/publications/${pub.slug}`} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-foreground/30 hover:text-blue-500 transition-colors">
                  <ImageIcon size={10} /> Abstract
                </Link>
                {pub.url && (
                  <a href={pub.url} target="_blank" className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-blue-500 hover:text-foreground transition-colors">
